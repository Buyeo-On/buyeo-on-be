...
class CurrentTermsIntegrationTests {
	...
	void unauthenticatedUserGetsLatestEffectiveTermForEachType() throws Exception {
		...
		UUID locationId = insertTerm("LOCATION", "1.0", true, "위치정보 약관", "위치정보 본문",
				Instant.parse("2026-07-01T00:00:00Z"));
		...
		mockMvc.perform(get("/terms")).andExpect(status().isOk())
				...
				.andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.items.length()").value(4))
				...
				.andExpect(jsonPath("$.data.items[2].termId").value(locationId.toString()))
