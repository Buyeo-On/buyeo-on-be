...
/** UC-27 부여 진입 알림 커맨드 서비스다. */
@Service
public class BuyeoEntryEventService {

	private static final String OPERATION = "NOTIFY_BUYEO_ENTRY";
	...
	public BuyeoEntryEventService(JdbcOperations jdbcOperations, PlatformTransactionManager transactionManager,
	...
	public BuyeoEntryEventView notify(UUID memberId, String idempotencyKey, BuyeoEntryEventCommand command) {
		validateIdempotencyKey(idempotencyKey);
		String requestHash = hash(command);
		return Objects.requireNonNull(
				transactions.execute(status -> notifyInTransaction(memberId, idempotencyKey, requestHash, command)),
				"부여 진입 알림 트랜잭션 결과가 없습니다.");
	}

	private BuyeoEntryEventView notifyInTransaction(UUID memberId, String idempotencyKey, String requestHash,
			BuyeoEntryEventCommand command) {
		...
		BuyeoEntryEventView result = new BuyeoEntryEventView(notificationSent);
	...
	/** UC-27의 검증 순서(경계 재검증 → 진행 중·미정산 여행 확인 → 쿨다운)를 그대로 따른다. */
	private boolean evaluateAndNotify(UUID memberId, BuyeoEntryEventCommand command, Instant now) {
		if (!boundary.covers(command.location().latitude(), command.location().longitude())) {
			return false;
		}
		if (tripQueryService.hasInProgressOrUnsettledTrip(memberId)) {
			return false;
		}
		if (notifications.existsByMemberIdAndTypeAndOccurredAtAfter(memberId, NotificationType.BUYEO_ENTRY,
				now.minus(COOLDOWN))) {
			return false;
		}
		notificationCreationService.createBuyeoEntry(memberId);
		return true;
	}

	private BuyeoEntryEventView replay(IdempotencyRequestEntity request, String requestHash) {
	...
	private String hash(BuyeoEntryEventCommand command) {
		try {
	...
	private String writeResponse(BuyeoEntryEventView result) {
		try {
			return objectWriter.writeValueAsString(SuccessResponse.of(result));
		} catch (JacksonException exception) {
			throw new IllegalStateException("부여 진입 알림 응답을 저장할 수 없습니다.", exception);
		}
	}

	private BuyeoEntryEventView readResponse(String responseBody) {
		try {
			JsonNode data = required(objectReader.readTree(responseBody), "data");
			return new BuyeoEntryEventView(requiredBoolean(data, "notificationSent"));
		} catch (JacksonException | IllegalArgumentException exception) {
			throw new IllegalStateException("저장된 부여 진입 알림 응답을 읽을 수 없습니다.", exception);
		}
	}
	...
	private JsonNode required(JsonNode node, String field) {
		...
		if (value == null) {
			throw new IllegalStateException("저장된 부여 진입 알림 응답 필드가 누락되었습니다: " + field);
		}
		return value;
	}
	...
	private boolean requiredBoolean(JsonNode node, String field) {
		...
		if (!value.isBoolean()) {
			throw new IllegalStateException("저장된 부여 진입 알림 응답 필드가 boolean이 아닙니다: " + field);
		}
		return value.booleanValue();
	}

	public record BuyeoEntryEventCommand(LocationCommand location) {
	}

	public record LocationCommand(double latitude, double longitude, Double accuracyMeters, OffsetDateTime capturedAt) {
	}

	public record BuyeoEntryEventView(boolean notificationSent) {
	}
}
