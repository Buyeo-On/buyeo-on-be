...
- 서버가 회원의 현재 위치를 경계·거리·미션·위치 알림 판정에 사용하면 `location_usage_records`에 회원, 사용 기능, 사용 시각과 만료 시각을 자동 기록한다.
- 확인자료에는 위도·경도·정확도 등 원시 좌표를 저장하지 않는다.
- 기록은 6개월 후 스케줄러가 파기하며 위치 동의 철회 또는 회원탈퇴 시 즉시 삭제한다.
...
- 업로드 URL 발급 시 예약을 저장하고 제출 완료 시 제거한다. 24시간이 지난 미제출 예약은 매시간 객체 삭제 후 제거하며, 실패하면 다음 실행에서 재시도한다.
- 탈퇴 시 미제출 예약 객체도 즉시 삭제하고 회원 연결이 제거된 예약을 24시간까지 남겨 지연 업로드를 다시 삭제한다. S3 버전 관리는 비활성화하며, 실패한 multipart upload는 버킷 수명 주기 규칙으로 정리한다.
