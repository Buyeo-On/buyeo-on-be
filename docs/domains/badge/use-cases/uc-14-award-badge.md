...
- 미션 완료는 `MISSION_COMPLETED_COUNT`를 판정하고, 같은 처리에서 방문 기록을 새로 만들었으면 `HERITAGE_VISITED_COUNT`도 한 번에 판정한다. 정답으로 제출한 객관식·OX 퀴즈는 같은 처리에서 `QUIZ_CORRECT_COUNT`도 함께 판정한다.
