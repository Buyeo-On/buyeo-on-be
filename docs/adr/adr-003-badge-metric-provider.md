...
- 지원 메트릭은 회원의 전체 여행을 누적하는 `MISSION_COMPLETED_COUNT`, `HERITAGE_VISITED_COUNT`, `POINT_DONATION_COUNT`, `PHOTO_SUBMISSION_COUNT`와 회원의 배지 획득 이력을 누적하는 `BADGE_ACQUIRED_COUNT`다.
  ...
  - 인증 사진 제출 수는 전체 여행에서 제출한 `PHOTO` 유형 mission submission 수다.
