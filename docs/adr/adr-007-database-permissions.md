...
- 공개 이미지 `image_url`을 `image_key`로 바꾸는 V5는 최초 production 배포 전에 한 번만 적용하는 비호환 migration으로 두고, V5 이전 앱으로의 rollback은 지원하지 않는다. 운영 시작 후 같은 종류의 변경은 expand/contract 단계로 나눈다.
